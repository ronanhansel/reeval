...
def init_MODEL_NAME2ID():
    global MODEL_NAME2ID
    if exists("CSV/model_df.csv"): model_df = pd.read_csv("CSV/model_df.csv")
    else:
        model_dfs = [get_list_models(f"helm_jsons/{b}") for b in BENCHMARKS]
    ...
    MODEL_NAME2ID = MappingProxyType(dict(zip(model_df['name'], model_df['model_id'])))

init_MODEL_NAME2ID()

def get_list_models(benchmark):
    schema_folder = "runs" if benchmark in VLM_BENCHMARKS else "releases"
    if exists(join(benchmark, schema_folder)):
        latest_release = sorted(os.listdir(join(benchmark, schema_folder)))[-1]
        schema_path = join(benchmark, schema_folder, latest_release, "schema.json")
        if exists(schema_path): 
            with open(schema_path, "r") as file:
                model_df = json.load(file)
...
def get_run_path(benchmark):
    if exists(join(benchmark, "releases")):
        latest_release = sorted(os.listdir(join(benchmark, "releases")))[-1]

        path = join(benchmark, "releases", latest_release, "runs_to_run_suites.json")
        if exists(path):
            with open(path, "r") as file:
                folder_dict = json.load(file)
            return [join(benchmark, "runs", suite, run) for run, suite in folder_dict.items()]
        
    elif exists(join(benchmark, "runs")):
        all_releases = sorted(os.listdir(join(benchmark, "runs")))
        
        run_paths = []
        for suite in all_releases:
            path = join(benchmark, "runs", suite, "runs_to_run_suites.json")
            if exists(path):
                with open(path, "r") as file:
                    folder_dict = json.load(open(path))

                for run, _ in folder_dict.items():
                    run_paths.append(join(benchmark, "runs", suite, run))

        return run_paths
        
    # In case the run paths are not found
    all_suites = os.listdir(join(benchmark, "runs"))
    # Filter out the suites that is folder and starting with letter "v"
    return [
        join(benchmark, "runs", suite)
        for suite in all_suites
        if os.path.isdir(join(benchmark, "runs", suite))
        and suite[0] != "v"
    ]

def process_run(run_path, scenarios, instances, instance2index):
    keys = [
        "model_id", "scenarios_id", "instance_id", "temperature", "num_completions",
        "top_k_per_token", "max_tokens", "stop_sequences", "echo_prompt", "top_p",
        "presence_penalty", "frequency_penalty", "predicted_text", "base64_images"
    ]
    responses = {key: [] for key in keys}

    # Get model and scenario info
    run_specs = json.load(open(join(run_path, "run_spec.json")))
    model_name = run_specs["adapter_spec"]["model"]
    model_id = MODEL_NAME2ID[model_name]
    
    delimiter = ":" if not run_specs["name"].startswith("legal_support") else ","
    scenario_name = run_specs["name"].split(delimiter)[0]
    ...
    if not exists(join(run_path, "instances.json")):
        return pd.DataFrame(responses)

    if exists(join(run_path, "display_requests.json")) and exists(join(run_path, "display_predictions.json")):
        return process_response_1(responses, model_id, scenario_id, scenario_name, run_path, instances, instance2index)

    if exists(join(run_path, "scenario_state.json")) and exists(join(run_path, "per_instance_stats.json")):
...
def process_response_1(responses, model_id, scenario_id, scenario_name, run_path, instances, instance2index):
    ...
    request_info = json.load(open(join(run_path, "display_requests.json")))
    prediction_info = json.load(open(join(run_path, "display_predictions.json")))
    instances_info = json.load(open(join(run_path, "instances.json")))
    stats = json.load(open(join(run_path, "stats.json")))
    
    # Get the list of metrics
    list_metrics = list(set([s["name"]["name"] for s in stats]))
    if "num_completions" in list_metrics: list_metrics.remove("num_completions")
    ...
    for request, prediction, per_inst in zip(request_info, prediction_info, instances_info):
        if not (request["instance_id"] == per_inst["id"] == prediction["instance_id"]): continue
        ...
        keys = [
            "temperature", "num_completions", "top_k_per_token", "max_tokens",
            "stop_sequences", "echo_prompt", "top_p", "presence_penalty", "frequency_penalty"
        ]
        for key in keys:
            responses[key].append(request["request"][key])
        
        responses["predicted_text"].append(prediction["predicted_text"])
        b64 = "base64_images"
        responses[b64].append(prediction[b64] if b64 in prediction else "")
...
def process_response_2(responses, model_id, scenario_id, scenario_name, run_path, instances, instance2index):
    # Read the prompt and prediction info
    request_info = json.load(open(join(run_path, "scenario_state.json")))["request_states"]
    prediction_info = json.load(open(join(run_path, "per_instance_stats.json")))
    instances_info = json.load(open(join(run_path, "instances.json")))
    stats = json.load(open(join(run_path, "stats.json")))
    ...
    for request, prediction, per_inst in zip(request_info, prediction_info, instances_info):
        ...
        for stat_name in list_metrics:
            ...
            responses[stat_name].append(prediction["stats"][all_avai_stats[stat_name]]["sum"])
...
if __name__ == "__main__":   
    for benchmark in BENCHMARKS:
        # if exists(f"CSV/{benchmark}/responses.pkl"): continue
        
        run_results = []
        scenarios = {"scenarios_id": [], "name": []}
        keys = [
            "instance_id", "scenarios_id", "instance_sid", "data_augmentation",
            "perturbation", "subject", "mode", "num_prompt_tokens", "prompt", "raw_question"
        ]
        instances = {key: [] for key in keys}
        instance2index = {}

        # Get the run paths and process them
        run_paths = get_run_path(f"helm_jsons/{benchmark}")
        if not run_paths: continue
        run_paths = sorted(run_paths)
        for run_path in tqdm(run_paths, desc=f"Processing {benchmark}"):
            if "instructions=none" in run_path: continue # Skip duplicate runs
            results = process_run(run_path, scenarios, instances, instance2index)
...
data = pickle.load(open(f"CSV/classic/responses.pkl", "rb"))