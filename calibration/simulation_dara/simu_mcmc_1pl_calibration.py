...
def irt_mcmc(
    question_num, testtaker_num, response_matrix, num_samples=9000, num_warmup=1000
):
    ...
    for i in tqdm(range(y.shape[1])):
        ...
        for j in range(len(bins) - 1):
            ...
            if bin_mask.sum() > 0:  # Bin not empty
                ...
                y_theoretical = item_response_fn_1PL(single_z3_samples, theta_mid)
...
def goodness_of_fit_1PL_plot(
    ...
    mean_diff, diff_array = goodness_of_fit_1PL(theta, z3_samples, y, bin_size)
    sample_means = []
    for _ in range(100):
...
if __name__ == "__main__":
    # wandb.init(project="simu_mcmc_1pl_calibration")
    ...
    output_dir = f"../data/simu_mcmc_1pl_calibration/{args.dataset}"
    plot_dir = f"../plot/simu_mcmc_1pl_calibration"
    ...
    theta_samples, z3_samples = irt_mcmc(question_num, testtaker_num, y)
