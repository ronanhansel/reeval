...
def em_2pl_calibration(
    ...
    z2_hat = (
        torch.distributions.LogNormal(0.0, 0.5)
        .sample((num_item,))
        .to(device)
        .requires_grad_(True)
    )
    ...
    for iter in pbar:
        ...
        if iter % 10 == 0:
            plot_corr(
                z2_hat_elu.clone().detach().numpy(),
                true_2pl_z2,
                f"../plot/plot_trace/{iter}_z2.png",
            )
            plot_corr(
                z3_hat_norm.clone().detach().numpy(),
                true_2pl_z3,
                f"../plot/plot_trace/{iter}_z3.png",
            )
...
if __name__ == "__main__":
    ...
    true_2pl_z2 = pd.read_csv("../data/pre_calibration/synthetic_2PL/z_true.csv")[
        "z2"
    ].values
    true_2pl_z3 = pd.read_csv("../data/pre_calibration/synthetic_2PL/z_true.csv")[
        "z3"
    ].values
    z2_hat, z3_hat = em_2pl_calibration(
        torch.tensor(y, dtype=torch.float64), true_2pl_z2, true_2pl_z3
    )
