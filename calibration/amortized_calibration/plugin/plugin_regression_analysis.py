...
from utils import DATASETS, error_bar_plot_double, goodness_of_fit_1PL
    ...
    for dataset in tqdm(DATASETS):
        ...
        for i in range(10):
            ...
            baseline_train_mse = mean_squared_error(
                z_train_true, np.repeat(z_baseline_pred, len(z_train_true))
            )
            baseline_test_mse = mean_squared_error(
                z_test_true, np.repeat(z_baseline_pred, len(z_test_true))
            )
